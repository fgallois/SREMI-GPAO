...
public class PdfService {
	...
	public void generatePdf(String invoiceNumber, String referenceCommand, 
				List<ItemValueObject> commands, File file) throws PdfException {
	...
	private Element createInformations() {
        ...
        paragraph.add(Chunk.NEWLINE);
        paragraph.add(new Phrase("N� Intracommunautaire: FR 26 439 754 581", FontFactory.getFont(FontFactory.TIMES_ROMAN, 12)));
        paragraph.add(Chunk.NEWLINE);
        paragraph.add(Chunk.NEWLINE);
        ...
        paragraph.add(Chunk.NEWLINE);
        paragraph.add(new Phrase("N� Intracommunautaire: FR 63 340 321 801", FontFactory.getFont(FontFactory.TIMES_ROMAN, 12)));
	...
	private Element createInfoCommand(String referenceCommand, String invoiceNumber) {
        ...
        cell = new PdfPCell(new Phrase(SimpleDateFormat.getDateInstance().format(new Date()), FontFactory.getFont(FontFactory.TIMES_ROMAN, 12)));
        cell.setMinimumHeight(20);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
        table.addCell(cell);
	...
	private Element createCommandTable(List<ItemValueObject> commands) {
        ...
        PdfPCell cell;
        for(ItemValueObject command: commands) {
        	if(command.isSelected()) {
                cell = new PdfPCell(new Phrase(new Integer(command.getCommandLine()).toString()));
                cell.setMinimumHeight(20);
                cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
                table.addCell(cell);

                cell = new PdfPCell(new Phrase(command.getItemReference()));
                cell.setMinimumHeight(20);
                table.addCell(cell);
                
                cell = new PdfPCell(new Phrase(command.getItemLabel()));
                cell.setMinimumHeight(20);
                table.addCell(cell);
